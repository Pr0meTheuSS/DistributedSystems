...
bool HttpManagerConnection::Send(const WorkerAnswer& answer)
{
    ...
    try {
        std::cout << userver::formats::json::ToStableString(answerJson) << std::endl;
        std::cout << this->m_url + "/internal/api/manager/hash/crack/request" << std::endl;
...
void HttpManagerConnection::sendPing(const SubTask& subTask)
{
    ...
    try {
        std::cout << userver::formats::json::ToStableString(subTaskJson) << std::endl;
        std::cout << this->m_url + "/internal/api/manager/subtask/status" << std::endl;
