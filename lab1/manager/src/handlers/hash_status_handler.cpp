...
HashStatusHandler::HashStatusHandler(
    ...
    case userver::server::http::HttpMethod::kGet: {
        ...
        if (!req) {
            ...
            throw userver::server::handlers::ClientError(
                userver::server::handlers::ExternalBody { fmt::format("Cannot find requst with uuid: {}", requestId) });
        }

        request.SetResponseStatus(userver::server::http::HttpStatus::kOk);
        CrackHashStatusResponse response { .requestId = req.value().id, .status = req.value().status };
