...
type CrackHashServiceInterface interface {
	...
	HandleDeadTasks(ctx context.Context) error
}
...
func (s *CrackHashService) HandleDeadTasks(ctx context.Context) error {
	pendingRequests, err := s.repository.GetAllPendingTasks(ctx)
	if err != nil {
		return err
	}
	for _, task := range pendingRequests {
		for i := int64(0); i < s.workersAmount; i++ {
			subTask := model.SubTask{
				ID:         uuid.NewString(),
				TaskID:     task.ID,
